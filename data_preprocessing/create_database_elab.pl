...
% database elaborato -> obesity with CNNs label 
start(obesitycnn) :-
    ensure_loaded(data_preprocessing/obesity_cnn/cnn_target_obesity_database),
    tell('data_preprocessing/obesity_cnn/cnn_target_obesity_database_elab.pl'),
    a(GENDER, AGE, HEIGHT, WEIGHT, FHO, FAVC, FCVC, NCP, CAEC, SMOKE, CH2O, SCC, FAF, TUE, CALC, MTRANS, TARGET),
	write('aa('), write(GENDER),
	write(','),  ( number(AGE), AGE =< 20,  write('\'0-20\'') ;
                   number(AGE), AGE > 20, AGE =< 30, write('\'21-30\'') ;
